...
public class WorkFlowParserUtil  {
    static Logger logger = Logger.getLogger(WorkFlowParserUtil.class);

//    todo(kgs): schema
    private static final String SCHEMA_LOCATION = "com/mentor/workflow/Workflow.xsd";
...
//        return new ClassPathResource(classPathFileLocation).getInputStream();
	...
	private void buildWorkFlowConfig(Set<WorkflowState> workflowSet,	List<ActionStateMapping> actions, Element tag) {
		...
		if (tagName.toLowerCase().equals("initial-state")) {
			processStateTag(workflowSet, tag, state,actions);
		}
		if (tagName.toLowerCase().equals("states")) {
			processStatesTag(workflowSet, tag, state,actions);
		}
	}

	private void processStatesTag(Set<WorkflowState> workflowSet, Element tag,WorkflowState state,List<ActionStateMapping> actions) {
		...
		for (int counter = 0; counter < states.getLength(); counter++) {
			...
			if (nNode instanceof Element) {
				...
				processStateTag(workflowSet, stateTag, state,actions);
			}
		}
	}

	private void processStateTag(Set<WorkflowState> workflowSet, Element tag, WorkflowState state,List<ActionStateMapping> actions) {
		String id = tag.getAttribute("id");
		state = getStateFromSet(id, workflowSet);
		...
		if (tag.getChildNodes() == null || tag.getChildNodes().getLength() == 0 ) {
			if (state == null) {
			    state = new WorkflowState(id, WorkflowStateType.FINAL);
			} else {
